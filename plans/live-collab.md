...
**AGREED NEXT STEP: the Y.Text document core (#13). Full migration plan written: [`ytext-core.md`](ytext-core.md).** Move the CRDT from the ProseMirror XmlFragment to a single `Y.Text` of markdown + inline CriticMarkup. It is the foundation that unblocks the rest: faithful save-back, a safe auto/periodic save and the unsaved-edits fix (#33), showing YAML in the editor again (#29), line-accurate scroll sync, and the cloud-bridge diff-merge (#9). The plan settles the why (the per-line paragraph join collapses `\n\n` to `\n`, so any live save of edited prose corrupts it), the target (text is the source of truth, save becomes an identity), the editor swap (CodeMirror 6 + `y-codemirror.next`, since `Y.Text` cannot back ProseMirror), the three hard parts (CriticMarkup as decorated literal delimiters, comment anchoring via `Y.RelativePosition`, the editor swap), the safe live-save write loop (Layer B: debounced conditional write + diff-merge), and a six-step de-risked rollout that keeps the app working throughout. High blast radius (collab core, CriticMarkup, comment/suggestion anchoring), so do it deliberately with tests. Read the plan, then start at rollout step 1 (the CM6 + Y.Text round-trip spike).
