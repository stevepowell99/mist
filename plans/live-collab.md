...
**AGREED NEXT STEP: the Y.Text document core (#13).** Move the CRDT from the ProseMirror XmlFragment to a single `Y.Text` of markdown + inline CriticMarkup. It is the foundation that unblocks the rest: faithful save-back, a safe auto/periodic save and the unsaved-edits fix (#33), showing YAML in the editor again (#29), line-accurate scroll sync, and the cloud-bridge diff-merge (#9). High blast radius (collab core, CriticMarkup, comment/suggestion anchoring), so do it deliberately with tests. Do this next.
