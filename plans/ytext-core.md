...
## SESSION OF 15 JUNE 2026 (evening). READ FIRST.

The #13 migration has been done for a while; this evening was a long run of features, fixes and a hosting wall. Latest deploy `b503a8dd` at mist.broad-smoke-cc64.workers.dev. All committed and pushed.

### THE BLOCKER right now: Durable Objects free-tier duration cap is EXHAUSTED.
`wrangler tail` shows every DO request failing with `Error: Exceeded allowed duration in Durable Objects free tier.` This is NOT a code bug. The deployed site cannot open or create docs (e.g. `/drive/import` 500s, websockets error) until the daily reset at **00:00 UTC**. Cause: heavy testing today, several tabs holding live websockets (each keeps a DO active and billing), and many redeploys. Levers: (1) close mist tabs, (2) wait for reset, (3) work on LOCALHOST (no Cloudflare cap), (4) the $5 Workers Paid plan removes the ceiling (Steve declined for now). The `/drive/import` 500 Steve saw is purely this cap, not a regression.

### Localhost setup (IN PROGRESS, the cap-free way to work solo)
`npm run dev` runs the Worker AND DOs locally in workerd (no cap), serves `http://localhost:5173`. Done: `.dev.vars` created (gitignored; `.dev.vars.example` documents the keys); `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` filled by Steve; `SESSION_SECRET` generated; `GOOGLE_SIGNIN_CLIENT_ID` fetched from prod `/auth/google`; redirect URI `http://localhost:53682/` added to the relay OAuth client `...ck5m0fhfg...`; JS origin `http://localhost:5173` added to the sign-in client `...1d50288j...`. STILL TO DO (Steve, when back at desk): (a) flip the OAuth consent screen from Testing to **Internal** (if causalmap.app is a Workspace) or **Publish**, else refresh tokens expire after 7 days; (b) then run `node scripts/get-refresh-token.mjs` (loopback consent on port 53682, writes `GOOGLE_REFRESH_TOKEN` into `.dev.vars`); (c) `npm run dev`, sign in, search. Console links and the OAuth-consent-mode note are in mist `CLAUDE.md`.

### Shipped this session (all deployed)
- **#7 Drive passphrase RETIRED.** `/drive/*` is Google sign-in (session cookie) + per-file Drive sharing only; sandboxed slides iframe uses the signed asset token. Deleted `drive-auth.server.ts` and `drive-key.ts`; removed all `X-Drive-Key`. `DRIVE_ACCESS_KEY` secret now unused.
- **#9 external-change sync.** On connect the DO re-reads Drive; if changed and no local divergence it pulls the new text into the Y.Text (prefix/suffix delta) and broadcasts; if both sides changed it shows a "Reload from Drive" affordance. `agents/document.ts`: `checkUpstream`, `replaceBodyFromText`, server-origin broadcast, `lastCommitMd` baseline at seed; provider now resyncs on EVERY (re)connect.
- **#12 idle auto-close.** Client drops the websocket after 5 min idle or 45 s hidden (PartySocket `close()` stops auto-reconnect), reconnects on key/pointer/wheel/focus; "Paused while idle" pill. This is the durable mitigation for the cap above.
- **#34 clipboard image paste.** Paste an image into the editor, uploads to an `img/` subfolder beside the doc, inserts `![](img/name)`. `driveCreateBinary` + `driveEnsureSubfolder`, route `/drive/upload`, CM6 paste handler.
- **Slides:** background images fixed (the bug was double-encoding: `driveAssetUrl` now encodes per path segment keeping literal `/`; the renderer re-encoded `%2F`), plus a stray `show()` removed (it was aborting `Reveal.layout()`, the cause of both blank previews and lost 2D layout). 2D section nesting (`groupSlides`) + `navigation-mode` from frontmatter. `.shot-cap` pinned bottom (`center:false` + full-height section). 5 s preview rebuild debounce. Reverse sync: navbar crosshair + Ctrl/Cmd+Alt+G jump the editor to the previewed slide (`offsetForSlideIndex`). Iframe keydown forwards mod+alt chords to the parent and `stopImmediatePropagation` so reveal's S/F/O do not double-fire.
- **Keyboard shortcuts, made reliable.** One `runChord` in `docs.$id.tsx` fed by THREE sources so focus never matters: a CodeMirror keydown handler (editor, stops propagation), a window bubble listener, and the slides-iframe postMessage. `modAltChord` (`app/lib/chord.ts`) matches `e.code` first (AltGr/numpad proof) and accepts the AltGraph modifier. E/S toggle edit/suggest; 1/2/3 view; O/C/F panels; G slide-to-editor; `/` help; **`-`/`=` resize the editor pane** (moved off `[`/`]`, which collide with CodeMirror fold-all). Help panel (Ctrl/Cmd+Alt+`/` or the `?` button) lists everything plus a Slide classes section.
- **Class picker** restored: inside `{ }` or after `:::`, typing `.` autocompletes classes parsed from the deck's CSS (`cssClasses` in DocumentContext, `cm-classes.ts`, shares one autocompletion with the `@` citations).
- **Default mode is now edit** (suggest-only links still forced to suggest).
- **Panels** (Drive sidebar, outline, comments-peek) slide in over ~300 ms instead of popping.
- **Defensive:** `/drive/search` always returns JSON (whole loader wrapped); `DriveBrowser` reads bodies via `readJson` so a non-JSON server error shows a clean message, not an "Unexpected token" crash. This made the cap error legible; it was not the cap's cause.

### Open follow-ups
- **Slides rebuild is slow** (full iframe reload re-runs reveal + plugins + re-renders all slides every edit). Added console instrumentation: `[mist-deck] reload Nms total | iframe Nms (mermaid Nms) | N slides`. WAITING on Steve to paste that line to choose the fix: if `total - iframe` dominates, keep the iframe alive and re-render in place (already prepped: `buildSlideSections` is split out of `buildSlidesHtml`, NOT yet wired to a postMessage in-place path with a hard-reload fallback); if `iframe` dominates, patch only the changed slide. Mermaid import is now skipped unless a deck has a mermaid block.
- **Could mist host others on their own Drives as a business?** Decided NO (the write-up was deleted at Steve's request). Single relay identity is the model; multi-tenant would need per-user tokens + Google verification; if ever revisited use the `drive.file` scope (avoids the restricted-scope audit), never full `drive`.

### Still pending tasks
#26 Zotero `@` extension, #27 preserve view/mode on opening from sidebar, #32 slide reorder in outline, #33 (effectively resolved by live save; closeable), #35 YAML header UX.
