...
| `theme:` | gmist theme CSS: `causal-map` (default), `qualia`, `brutalist`, `editorial`, `blackboard`, `moonshot`, `handwritten`, `minimal`. An unknown value (e.g. an old reveal theme like `serif`/`black`) falls back to causal-map. Themes are plain files in `app/styles/themes/*.css`, resolved in `app/lib/themes.ts`, injected after `deck-base.css` and before any `css:`. The same theme CSS drives the document Preview, so a doc reads like its deck. |
