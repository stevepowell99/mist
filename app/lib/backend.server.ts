 ...
 *
 * A backend instance is bound to one document's storage location (a Google Drive
 * file). The relay, the import route and the folder sidebar talk to storage only
 * through this interface, so the storage details stay in one place. See
 * plans/live-collab.md.
 *
 * Folder refs are opaque strings the backend understands: a Drive folder id. The
 * sidebar passes them back to list() and parentRef() without interpreting them.
 */
...
export interface DocBackend {
  /** Current text plus a version token (Drive headRevisionId), null if none. */
  read(): Promise<{ text: string; version: string | null }>;
  /**
   * Write new text. `expectedVersion` is the version the session loaded with,
   * for a conditional write. Returns the new version token.
   */
  write(
    text: string,
  ...
  // Folder navigation (Drive folders).
