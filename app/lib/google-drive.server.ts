/**
 * Google Drive storage implementation (server-only).
 *
 ...
 *
 * Call sites go through the google.server.ts facade, which dispatches between
 * this module and google-localfs.server.ts; do not import this one directly.
 */
import {
  FOLDER_MIME,
  DOC_MIME,
...
/** File text content plus its version token. Raw bytes as text; the facade
 *  canonicalises line endings (the LF invariant lives there). */
...
// so this is a post-filter on the resolved path. (The localfs sidecar,
// scripts/localfs-server.mjs, mirrors this list for its index walk.)
